...
url_ini<-read_html('http://www.valasztas.hu/dyn/pv14/szavossz/hu/TK/szkkivtka.html')
...
urls_highlevel<-paste0('http://www.valasztas.hu/dyn/pv14/szavossz/hu/TK/',urls_highlevel)
...
urls_midlevel<-paste0('http://www.valasztas.hu/dyn/pv14/szavossz/hu/',urls_midlevel)
...
#this should have a list of 10386 elements, with the urls of the result page for each individual voting place ("szavazókör")
...
#example table
temp_table<-valkor_results_final[[512]][[7]]
View(temp_table)
...
get_val_kor_results<-function(indiv_valkor_tables_list) {
        ...
        for (i in 2:length(indiv_valkor_tables_list)) {
                ...
                if (temp_table[1,1]=='Összesen'&length(temp_table[1,])==7) {temp_df[1,c('SZVLP_LST','SZVLP_LST_LTR','ERVTL_LST','ERV_LST'):=as.list(as.character(temp_table[2,4:7]))]}
                if (temp_table[1,1]=='Összesen'&length(temp_table[1,])==8) {temp_df[1,c('SZVLP_LST','SZVLP_LST_LTR','ERVTL_LST','ERV_LST'):=as.list(as.character(temp_table[1,5:8]))]}
                if (temp_table[1,1]=='Összesen'&length(temp_table[1,])==8) {temp_df[1,KULKEP_ATJEL:=(as.character(temp_table[1,4]))]}
                
                if (names(temp_table)[2]=='A jelölt neve') {
                names(temp_table)<-c('sorszam','nev','szervezet','szav')
                
                temp_df[,'FIDESZ_egy']<-temp_table[temp_table$szervezet=='FIDESZ-KDNP','szav']
                temp_df[,'FIDESZ_egy_jlt']<-temp_table[temp_table$szervezet=='FIDESZ-KDNP','nev']
                
                temp_df[,'MSZP_egy']<-temp_table[temp_table$szervezet=='MSZP-EGYÜTT-DK-PM-MLP','szav']
                temp_df[,'MSZP_egy_jlt']<-temp_table[temp_table$szervezet=='MSZP-EGYÜTT-DK-PM-MLP','nev']
                
                temp_df[,'JOBBIK_egy']<-temp_table[temp_table$szervezet=='JOBBIK','szav']
                temp_df[,'JOBBIK_egy_jlt']<-temp_table[temp_table$szervezet=='JOBBIK','nev']
                
                temp_df[,'LMP_egy']<-temp_table[temp_table$szervezet=='LMP','szav']
                temp_df[,'LMP_egy_jlt']<-temp_table[temp_table$szervezet=='LMP','nev']
                }
                ...
                temp_df[,'FIDESZ_lst']<-temp_table[temp_table$nev=='FIDESZ-KDNP','szav']
                temp_df[,'MSZP_lst']<-temp_table[temp_table$nev=='MSZP-EGYÜTT-DK-PM-MLP','szav']
                temp_df[,'JOBBIK_lst']<-temp_table[temp_table$nev=='JOBBIK','szav']
                temp_df[,'LMP_lst']<-temp_table[temp_table$nev=='LMP','szav']
                }
        }
...
valkor_results_df[,MAS_egy:=NE-(FIDESZ_egy+MSZP_egy+JOBBIK_egy+LMP_egy)]
valkor_results_df[,MAS_egy_pct:=MAS_egy/NE]

#calculating "other" votes (not main parties), and their percent share for lists
valkor_results_df[,MAS_lst:=ERV_LST-(FIDESZ_lst+MSZP_lst+JOBBIK_lst+LMP_lst)]
...
valker_links<-getlinks_from_t2('http://www.valasztas.hu/dyn/pv14/szavossz/hu/oevker.html')

valker_links<-valker_links[grepl('M',valker_links)]

valker_links<-paste0('http://www.valasztas.hu/dyn/pv14/szavossz/hu/',valker_links)

valker_names<-read_html('http://www.valasztas.hu/dyn/pv14/szavossz/hu/oevker.html')%>%html_table()
...
valke_data_names<-c('MEGYE','OEVK','SZEKHELY','AE','BE','CE','EE','FE','IE','JE','JE_pct','KE','ME','ME_pct','NE',  'FIDESZ_egy','MSZP_egy','JOBBIK_egy','LMP_egy','MAS_egy','FIDESZ_egy_pct','MSZP_egy_pct','JOBBIK_egy_pct','LMP_egy_pct','MAS_egy_pct', 'FIDESZ_lst','MSZP_lst','JOBBIK_lst','LMP_lst','MAS_lst','FIDESZ_lst_pct','MSZP_lst_pct','JOBBIK_lst_pct','LMP_lst_pct','MAS_lst_pct','FIDESZ_egy_jlt','MSZP_egy_jlt','JOBBIK_egy_jlt','LMP_egy_jlt')
...
get_valker_results<-function(table_list) {
        ...
        temp_df[,'MSZP_egy']<-temptable[temptable$szervezet=='MSZP-EGYÜTT-DK-PM-MLP','szav']
        temp_df[,'MSZP_egy_pct']<-temptable[temptable$szervezet=='MSZP-EGYÜTT-DK-PM-MLP','pct']
        temp_df[,'MSZP_egy_jlt']<-temptable[temptable$szervezet=='MSZP-EGYÜTT-DK-PM-MLP','nev']
