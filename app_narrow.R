...
valker_result_df<-data.table(read.csv('valker_results_utf8.csv',fileEncoding = "UTF-8"))
...
get_seats_const_fidesz_plus_coord<-function(voting_share=61.26499,J_to_M,J_to_F,J_to_L,M_to_J,M_to_L,L_to_M,L_to_J) {
       ...
       seats_summary[3,2:6]<-seats[,FIDESZ_sum:MAS_sum]
        ...
        margin_over_FIDESZ<- margin_over_FIDESZ[,OPP_max:=NULL]
        ...
        margin_over_FIDESZ<-margin_over_FIDESZ[,names(margin_over_FIDESZ[,MSZP_egyedul_mrg:LMP_egyedul_mrg]):=lapply(.SD, as.integer),.SDcols=MSZP_egyedul_mrg:LMP_egyedul_mrg]

        setnames(margin_over_FIDESZ,old=names(margin_over_FIDESZ[,baseline_winner:LMP_egyedul_mrg]),new=c('Alapeset győztes','Koordinációnál legesélyesebb','Koordinációnál második legesélyesebb','Koordinációnál harmadik legesélyesebb', 'MSZP marzsa egyedül induláskor','Jobbik marzsa egyedül induláskor','LMP marzsa egyedül induláskor'))
...
ui <- fluidPage(
   ...
   fluidRow(
              inputPanel(
                      ...
                      actionButton("update", HTML("Lássuk az eredményt!<br/>(Számold újra!)"),style="color: #fff; background-color: #337ab7; border-color: #2e6da4"),
